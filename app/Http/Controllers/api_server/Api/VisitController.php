...
class VisitController extends Controller
{
    ...
    // Ubah nilai di sini jika aturan bisnis berubah — tidak perlu update aplikasi mobile.
    //
    ...
    // ============================================================================
    const BATAS_HARI_KERJA_LIST = 5;   // ← ubah angka ini jika perlu
    ...
    // Aplikasi lama (v5/v6) tidak mengirim header X-App-Version → ditolak
    // Aplikasi baru (v8+) mengirim header ini via ApiHttp → diterima
    // Return: null = OK, Response = tolak dengan 426
    // ============================================================================
    ...
    {
        Log::info('📊 GET CALL LIST DATA - REQUEST', [
            'id_peg' => $req->input('id_peg'),
        ...
        // Jika periode diberikan, filter berdasarkan tahun dan bulan yang sama
        if ($periode) {
            ...
            Log::info('📊 GET CALL LIST DATA - FILTER APPLIED', [
                'year' => $year,
                'month' => $month,
            ]);
        }
        ...
        Log::info('📊 GET CALL LIST DATA - RESULT', [
            'count' => $data->count(),
        ]);
    ...
    public function getCallList(Request $req)
    {
        ...
        // LEFT JOIN dengan call_list untuk filter customer yang sudah ada
        // Hanya tampilkan customer yang BELUM ada di call_list untuk periode yang sama
        ...
        if (!empty($idPeg)) {
            ...
            if (is_array($decoded)) {
                ...
                // $query->where(function ($q) use ($decoded, $struktur) {
                //     $q->whereIn('master_dokter.id_peg', $decoded)
                //         ->orWhereIn('master_dokter.id_peg', explode(',', $struktur->id_peg));
                // });
                ...
                // $query->where(function ($q) use ($idPeg, $struktur) {
    ...
    public function getMonthlyCount(Request $req)
    {
        ...
        // Return total + per-tipe (Dokter & Non-Dokter)
        // Digunakan Flutter untuk cek limit per tipe sebelum save
    ...
    public function saveCallList(Request $req)
    {
        ...
        // Nilai diambil dari konstanta kelas di atas (BATAS_HARI_KERJA_LIST & OVERRIDE_BULAN_LIST).
        // Untuk mengubah: cukup edit nilai konstanta di bagian atas file ini.
        // ============================================================
    ...
    {
        Log::info('CALL PLAN REQUEST = ', $req->all());
        ...
        Log::info("FOUND DOCTORS: " . $doctors->count());
    ...
    public function callPlanInst(Request $req)
    {
        ...
        Log::info('callPlanInst DEBUG', [
            'id_mcl_received' => $idMcl,
            'id_ff_received' => $idFF,
            'id_mcl_int' => (int)$idMcl
        ]);

        // if (!empty($idFF)) {
        //     $getjabatan = DB::table('data_pegawai')->select('jabatan')->where('id', $idFF)->where(DB::raw('ifnull(status,"Exist")'), 'Exist')->first();
        // }
        ...
        $results = DB::table('list_dokter_visit_new as z')
            ...
            // ->where('id_ff', $idFF)
            // ->where(function ($q) use ($idFF) {
            //     $q->where('z.id_ff', $idFF)
            //         ->orWhereRaw('z.id_ff in (select id_mr from struktur where (id_mr="' . $idFF . '" or id_dm="' . $idFF . '" or id_rsm="' . $idFF . '") and periode_akhir="' . date('Y-m-20') . '")');
            // })
    ...
    // Ambil daftar produk aktif dari data_product untuk dipilih saat add call plan
    public function getProductList()
    {
        $products = DB::table('data_product')
            ->where('status', 'AKTIF')
            // ->select('id_product', 'nama_product', 'jenis_product', 'kemasan')
            ->select(
                        'id_product',
                        'nama_product',
                        'jenis_product',
    ...
    public function saveActual(Request $req)
    {
        ...
        // $clientTime = Carbon::parse($req->actual_time_utc);
        // if (abs($clientTime->diffInMinutes(now('UTC'), false)) > 120) {
        //     return response()->json([
        //         'success' => false,
        //         'error_code' => 'INVALID_TIME_SETTING',
        //         'message' => 'Anda merubah tanggal device, sesuaikan dengan tanggal sekarang.'
        ...
        // ============================================================
        // Lanjut proses simpan jika masih dalam batas waktu
        // ============================================================
        ...
        try {
            ...
            $updateData = [
                ...
                // simpan foto & tanda_tangan langsung di save utama
                // agar atomik — jika frontend kirim nilai, tersimpan dalam 1 transaksi
                // (sebelumnya hanya disimpan via endpoint terpisah /actual-save-photo)
                'foto'          => $req->foto ?? null,
    ...
    private function createJoinVisitRecordsForAtasan(int $mrRecordId, string $joinVisitIdStr, bool $hasUpdatedDate): void
    {
        ...
        foreach ($atasanPegIds as $atasanPegId) {
            ...
            // Tidak mengambil id_ff/divisi karena mungkin tidak ada di tabel data_pegawai
            ...
            Log::info("[Join Visit] Record created for atasan id_peg={$atasanPegId}, mr_record_id={$mrRecordId}");
        }
    }
    ...
    public function saveUnplanned(Request $req)
    {
        ...
        // Toleransi waktu 2 jam untuk multi waktu di indonesia (misal MR di WIT, server di WIB → selisih 1-2 jam)
        // $clientTime = Carbon::parse($req->actual_time_utc);
        // $clientTime = Carbon::parse($req->tgl_actual);
        // if (abs($clientTime->diffInMinutes(now('UTC'), false)) > 120) {
        //     return response()->json([
        //         'success' => false,
        //         'error_code' => 'INVALID_TIME_SETTING',
        ...
        try {
            ...
            $insertData = [
                ...
                'product_list'        => $req->product_list ?? null,
                // simpan foto & tanda_tangan langsung saat insert
                // agar atomik — jika frontend kirim nilai, tersimpan bersama data lain
                'foto'                => $req->foto ?? null,
                'tanda_tangan'        => $req->tanda_tangan ?? null,
            ];
    ...
    // Parameter: id_mcl, id_peg, periode
    ...
    public function deleteCallList(Request $req)
    {
        try {
            ...
            Log::info('🗑️ DELETE CALL LIST REQUEST:', [
                'id_mcl' => $req->id_mcl,
                'id_peg' => $req->id_peg,
                'periode' => $req->periode,
            ]);
            ...
            Log::info('📅 PARSED PERIODE:', [
                'year' => $periodeYear,
                'month' => $periodeMonth,
            ]);

            //  CEK APAKAH DATA SUDAH ADA DI CALL PLAN (dengan perbandingan YEAR-MONTH)
            // Data hanya bisa dihapus jika belum ada di call_plan_actual
            // dengan id_mcl + id_peg + YEAR-MONTH yang sama
            ...
            Log::info('🔍 CHECK CALL PLAN EXISTENCE:', [
                'exists' => $existsInPlan,
                'id_mcl' => $req->id_mcl,
                'id_peg' => $req->id_peg,
                'year' => $periodeYear,
            ...
            if ($callListRecord && $callListRecord->approval === 'Reject') {
                Log::warning('⚠️ DELETE BLOCKED: Status is Rejected — must edit instead', [
                    'id_mcl' => $req->id_mcl,
                    'id_peg' => $req->id_peg,
                ]);
                return response()->json([
    ...
    // Parameter: id (id dari call_plan_actual table)
    ...
    public function getReachProdReport(Request $req)
    {
        ...
        // (lihat doctorProductivityPercentage di visit_call_report_page.dart: kalau targetNonDokter==0 → hanya prodDoctor)
    ...
    public function getFreqReport(Request $req)
    {
        ...
        $data = $query->select(
            ...
            // ➡️ FIX: count ONLY approved actuals
            DB::raw("(
                select count(id)
            ...
            // ➡️ FIX: point only if approved actual >= target
            DB::raw("case
                when (
                    select count(id)
                    from call_plan_actual
    ...
    public function getCallListTarget(Request $req)
    {
        try {
            ...
            Log::info('📊 GET CALL LIST TARGET - REQUEST', [
                'id_peg' => $idPeg,
                'periode' => $periode,
            ]);
            ...
            Log::info('📊 GET CALL LIST TARGET - PEGAWAI INFO', [
                'jabatan' => $pegawai->jabatan,
                'divisi' => $pegawai->divisi,
            ]);

            // Query target dari tabel call_target_list
            // Filter berdasarkan: jabatan, divisi, dan periode (periode_awal <= periode <= periode_akhir)
            ...
            Log::info('📊 GET CALL LIST TARGET - RESULT', [
                'target_found' => $target ? true : false,
                'dokter' => $target->dokter ?? 0,
                'non_dokter' => $target->non_dokter ?? 0,
            ]);
    ...
    public function getWorkingDays(Request $request)
    {
        ...
        Log::info("📅 GET WORKING DAYS - Input: year=$year, month=$month");
        ...
        Log::info("📅 GET WORKING DAYS - Formatted period: $bulanFormatted");

        // Query database report_admin_mkt.set_param_sum_mcr
        // Ambil nilai hari_kerja yang sudah berisi total (e.g., 20, 21, 22)
        ...
        // 🔍 DEBUG: Log result
        Log::info("📅 GET WORKING DAYS - Result: $workingDays days");
        ...
        if ($idPeg) {
            ...
            // Log::info("📅 GET WORKING DAYS - Unvisit count for id_peg=$idPeg, periode=$periodeFormatted2: mr=$countMr, nonMr=$countNonMr, total=$unvisitCount");
        }
        ...
        $response = [
           ...
           //tambahan field untuk unvisit deduction
            ...
            // 🔍 DEBUG: Tambahkan debug info ke response
            'debug' => [
                'input_year' => $year,
                'input_month' => $month,
    ...
    public function getProductivityTarget(Request $req)
    {
        ...
        try {
            ...
            Log::info('📊 GET PRODUCTIVITY TARGET - target_hari', [
                'jabatan'          => $pegawai->jabatan,
                'divisi'           => $pegawai->divisi,
                'periode'          => $periodeStr,
                'dokter_per_day'   => $dokterPerDay,
                'non_dokter_per_day' => $nonDokterPerDay,
            ]);
            ...
            Log::info('📊 GET PRODUCTIVITY TARGET - working days', [
                'working_days'    => $workingDays,
                'unvisit_count'   => $unvisitCount,
    ...
    /**
     * Konfigurasi batas tanggal unvisit — satu tempat untuk semua referensi.
     * Ubah nilai di sini jika kebijakan berubah; tidak perlu update Dart maupun
     * memodifikasi validasi addUnvisit secara terpisah.
     */
     ...
     * Aplikasi Dart menggunakan nilai ini agar date-picker menyesuaikan
     * kebijakan yang dikelola di sisi server.
     */
     ...
     * List dipelihara di sini sehingga Dart tidak perlu di-update
     * jika ada tambahan/perubahan alasan.
     */
     ...
     * Mengambil list unvisit berdasarkan id_peg (array JSON) dan periode.
     */
    ...
    /**
     * POST /delete-unvisit
     * Menghapus data unvisit berdasarkan id.
     * Mencoba menghapus dari kedua tabel (mr dan non-mr).
     */
    ...
    // ============================================================
    // UPDATE CALL LIST (DENGAN AUDIT TRAIL)
    // Hanya bisa edit jika status = Pending atau Reject
    // ============================================================
    public function updateCallList(Request $req)
    {
        ...
        //     return response()->json([
        //         'success' => false,
        //         'message' => 'Cannot edit! Status is already Approved.'
        //     ], 403);
        // }
        ...
        try {
            ...
            Log::info(" Call List Updated - ID: {$req->call_list_id}, Changes: " . json_encode(array_keys($changes)));
    ...
    // Endpoint ini dibaca oleh ApprovalNotificationService di Flutter setiap 30 menit.
    // Berbeda dengan DmApprovalNotificationSummary (untuk DM yg approve bawahan),
    // endpoint ini untuk MR/user sendiri — berapa banyak item call list mereka
    // yang sudah disimpan tapi belum di-approve oleh DM.
    //
    // Dipanggil via: POST /get-my-pending-call-list-count
    // Body JSON: { "id_peg": [123, 456], "month": 4, "year": 2026 }
    // ============================================================================
