...
class JoinVisitController_dev extends Controller
{
    // KONFIGURASI RADIUS JOIN VISIT
    const JOIN_VISIT_RADIUS_METERS = 120; // ubah angka ini sesuai kebutuhan

    // hitungJarak — Haversine formula, return jarak dalam meter
    ...
    // getAppConfig — mengembalikan konfigurasi yang bisa diatur dari server
    public function getAppConfig()
    {
        return response()->json([
            'success' => true,
            'data' => [
                'join_visit_radius_meters' => self::JOIN_VISIT_RADIUS_METERS,
            ]
        ]);
    }
    ...
    public function approvalJoinVisit(Request $req)
    {
        ...
        $employees = DB::table('call_plan_actual as cl')
            ...
            ->whereRaw('cl.updated_date >= NOW() - INTERVAL 180 MINUTE') //setting waktu join visit
    ...
    public function joinVisitDetails(Request $req)
    {
        ...
        $year  = $req->input('year');
        $approverId = $req->input('approver_id');
        ...
        $requests = DB::table('call_plan_actual as cpa')
            ...
            ->whereRaw('cpa.updated_date >= NOW() - INTERVAL 180 MINUTE') //setting waktu join visit
    ...
    public function copyJoinVisit(Request $req)
    {
        $id        = $req->input('id');
        $idPeg     = $req->input('id_peg');
        $koorVisit = $req->input('koor_visit'); // atasan's position — bisa array (Position JSON dari Flutter) atau "lat,long" string
        ...
        // ============================================================
        // VALIDASI RADIUS — atasan harus dalam jangkauan JOIN_VISIT_RADIUS_METERS dari MR
        // Flutter mengirim koor_visit sebagai string "lat,lng"
        // ============================================================
        $mrKoor = DB::table('call_plan_actual')->where('id', $id)->value('koor_visit');

        if (!empty($mrKoor)) {
            // Koordinat lokasi atasan wajib ada jika MR punya koor_visit
            if (empty($koorVisit) || !is_string($koorVisit) || !str_contains($koorVisit, ',')) {
                return response()->json([
                    'success' => false,
            ...
            // Parse koordinat MR dari DB (format: "lat,lng")
            $mrParts = explode(',', $mrKoor);
            // Parse koordinat atasan dari request (format: "lat,lng")
            $ap = explode(',', $koorVisit);

            if (count($mrParts) === 2 && count($ap) === 2) {
                $mrLat     = (float) trim($mrParts[0]);
                $mrLng     = (float) trim($mrParts[1]);
                $atasanLat = (float) trim($ap[0]);
                $atasanLng = (float) trim($ap[1]);

                $jarak = $this->hitungJarak($atasanLat, $atasanLng, $mrLat, $mrLng);

                if ($jarak > self::JOIN_VISIT_RADIUS_METERS) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Lokasi Anda terlalu jauh dari lokasi MR pada saat input Visit ('
                            . round($jarak) . 'm). Maksimum: ' . self::JOIN_VISIT_RADIUS_METERS-20 . 'm.',
                    ], 422);
                }
            }
        }
