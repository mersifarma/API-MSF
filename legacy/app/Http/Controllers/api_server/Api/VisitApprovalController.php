...
class VisitApprovalController extends Controller
{
    ...
    // --- CALL LIST ---
    const BATAS_HARI_KERJA_LIST = 5;
    // Isi format 'YYYY-MM' (contoh: '2026-04') untuk skip validasi pada bulan itu => Kosongkan ('') jika tidak ada override.
    const OVERRIDE_BULAN_LIST   = '';

    // --- CALL PLAN ---
    // Batas jam approval call plan di hari yang sama dengan tgl_plan (format 24 jam WIB).
    const BATAS_JAM_PLAN        = 10;

    // --- CALL ACTUAL ---
    const BATAS_HARI_ACTUAL     = 1;
    ...
    const BATAS_JAM_ACTUAL      = 10;   // ← ubah angka jika perlu

    // --- NOTIFIKASI REMINDER ---
    // Interval pengecekan approval di aplikasi Flutter (dalam menit).
    // Ubah nilai ini untuk mengatur seberapa sering reminder muncul — tanpa perlu update aplikasi.
    const NOTIFICATION_INTERVAL_MINUTES = 1; //Untuk setting notifikasi reminder, default 30 menit. Ubah sesuai kebutuhan.
    ...
    // ============================================================================
    // APPROVAL CALL LIST - Approval untuk tabel call_list
    // ============================================================================

    //get approval name for call list
    public function DmApprovalListName(Request $req)
    {
        ...
        // tidak perlu query — langsung kembalikan empty (tidak ada yang perlu di-approve).
        // ============================================================
    ...
    public function DmApprovalListDetails(Request $req)
    {
        ...
        if (self::BATAS_HARI_KERJA_LIST !== null && empty($cekpegawai->rowid)) {
            if (empty(self::OVERRIDE_BULAN_LIST) || trim(self::OVERRIDE_BULAN_LIST) !== $periodeString) {
    ...
    public function DmApprovalListSave(Request $req)
    {
        ...
        // Nilai diambil dari konstanta kelas di atas (BATAS_HARI_KERJA & OVERRIDE_BULAN).
        ...
        foreach ($approvals as $item) {
            ...
            // Simpan approval_comment saat Reject
            // agar karyawan tahu alasan reject saat membuka form edit
    ...
    // ============================================================================
    // APPROVAL CALL PLAN - Approval untuk tgl_plan di tabel call_plan_actual
    // Columns: approval, approval_by, approval_date, approval_comment
    // ============================================================================

    // Get employees with pending plan approval requests
    ...
    // ============================================================================
    // APPROVAL CALL ACTUAL - Approval untuk tgl_actual di tabel call_plan_actual
    // Columns: approval_actual, approval_actual_by, approval_actual_date, approval_actual_comment
    // ============================================================================
    ...
    public function DmApprovalActualName(Request $req)
    {
        ...
        if (self::BATAS_HARI_ACTUAL !== null && empty($cekpegawai->rowid)) {
            ...
            if (self::BATAS_JAM_ACTUAL !== null && $this->sudahLewatJam(self::BATAS_JAM_ACTUAL)) {
                ...
                $cutoffTglActual = $now->copy()->subDays((self::BATAS_HARI_ACTUAL - 1) + $extraDays)->format('Y-m-d');
            } else {
    ...
    public function DmApprovalActualDetails(Request $req)
    {
        ...
        // ============================================================
        // [CONFIG] HITUNG cutoff tgl_actual (sama seperti DmApprovalActualName)
        // ============================================================
        ...
        $requests = DB::table('call_plan_actual as cpa')
            ...
            // [CONFIG] Hanya tampilkan yang belum expired
            ->when($cutoffTglActual !== null, fn($q) => $q->where('cpa.tgl_actual', '>=', $cutoffTglActual))
            ->select(
                'cpa.id',
                'cpa.tgl_plan',
                'cpa.waktu',
                ...
                'cpa.status',
                'cpa.foto',
                // Tambah join_visit flag untuk badge JV di list
                'cpa.join_visit',
                'cpa.join_visit_ff'
            )
            ...
            DB::transaction(function () use ($approvals, $dmPeg, $now, $today, $cekpegawai) {
                ...
                foreach ($approvals as $item) {
                    ...
                    $actual = DB::table('call_plan_actual')
                        ->select('id_mcl', 'id_peg', 'tgl_actual', 'foto')
                        ->where('id', $id)
                        ->first();
                    ...
                    // ============================================================
                    $approved = ($item['approval_actual'] ?? '') === 'Approve';
                    if ($approved && (is_null($actual->foto) || trim($actual->foto) === '')) {
    ...
    // Endpoint baru untuk approval satu-satu (bukan batch)
    // Digunakan dari halaman view_approval_actual_detail_page.dart
    // ============================================================================
    public function DmApprovalActualSingle(Request $req)
    {
        ...
        try {
            DB::transaction(function () use ($id, $dmPeg, $approvalStatus, $comment, $now, $cekpegawai) {
                // Ambil data actual
                $actual = DB::table('call_plan_actual')
                    ->select('id_mcl', 'id_peg', 'tgl_actual', 'foto')
                    ->where('id', $id)
                ...
                // VALIDASI FOTO: Approve tidak bisa dilakukan jika foto null atau kosong
                // ============================================================
                if ($approvalStatus === 'Approve' && (is_null($actual->foto) || trim($actual->foto) === '')) {
    ...
    // Endpoint khusus untuk notifikasi reminder approval di Flutter.
    // Mengembalikan jumlah pending approval (call list, plan, actual) + info deadline.
    // Dipanggil saat app dibuka dan setiap 30 menit oleh ApprovalNotificationService.
    // ============================================================================
    public function DmApprovalNotificationSummary(Request $req)
    {
        ...
        $cekList = DB::table('call_setting as a')
            ->join('data_pegawai as b', 'a.user', '=', 'b.id_user')
            ->select('b.rowid', 'a.jumlah')
            ->whereIn('b.rowid', $idPegList)
            ->where('a.input_set', 'Approval Call List')->first();
        ...
        if (!$listExpired) {
            if (str_contains($jabatan, 'ACT. DM') || str_contains($jabatan, 'DM')) {
                $listCount = DB::table('call_list as cl')
                    ->join('struktur as s', fn($j) => $j->on('s.id_peg_mr', '=', 'cl.id_peg')->orOn('s.id_peg_dm', '=', 'cl.id_peg'))
                    ->leftJoin('data_pegawai as rsm', 'rsm.rowid', '=', 's.id_peg_rsm')
                    ->leftJoin('data_pegawai as mm',  'mm.rowid',  '=', 's.id_peg_mm')
                    ->whereRaw('? BETWEEN s.periode_awal AND s.periode_akhir', [$monthyear])
                    ->whereIn('s.id_peg_dm', $idPegList)->whereNull('cl.approval')
                    ->whereBetween('cl.periode', [$startDate, $endDate])
                    ->where(fn($q) => $q->whereColumn('cl.id_peg', 's.id_peg_mr')
                        ->orWhere(fn($q2) => $q2->whereColumn('cl.id_peg', 's.id_peg_dm')
                            ->whereIn('rsm.status', ['Vacant', 'Dummy'])->whereIn('mm.status', ['Vacant', 'Dummy'])))
                    // ->count('cl.id');
                    ->distinct()->count('cl.id');
            } elseif (str_contains($jabatan, 'RSM')) {
                $listCount = DB::table('call_list as cl')
                    ->join('struktur as s', fn($j) => $j->on('s.id_peg_dm', '=', 'cl.id_peg')->orOn('s.id_peg_mr', '=', 'cl.id_peg')->orOn('s.id_peg_rsm', '=', 'cl.id_peg'))
                    ->leftJoin('data_pegawai as dm', 'dm.rowid', '=', 's.id_peg_dm')
                    ->leftJoin('data_pegawai as mm', 'mm.rowid', '=', 's.id_peg_mm')
                    ->whereRaw('? BETWEEN s.periode_awal AND s.periode_akhir', [$monthyear])
                    ->whereIn('s.id_peg_rsm', $idPegList)->whereNull('cl.approval')
                    ->whereBetween('cl.periode', [$startDate, $endDate])
                    ->where(fn($q) => $q->whereColumn('cl.id_peg', 's.id_peg_dm')
                        ->orWhere(fn($q2) => $q2->whereColumn('cl.id_peg', 's.id_peg_mr')->whereIn('dm.status', ['Vacant', 'Dummy']))
                        ->orWhere(fn($q3) => $q3->whereColumn('cl.id_peg', 's.id_peg_rsm')->whereIn('mm.status', ['Vacant', 'Dummy'])))
                    // ->count('cl.id'); 
                    ->distinct()->count('cl.id');
            } elseif (str_contains($jabatan, 'MM')) {
                $listCount = DB::table('call_list as cl')
                    ->join('struktur as s', fn($j) => $j->on(fn($q) => $q->on('s.id_peg_dm', '=', 'cl.id_peg')->orOn('s.id_peg_mr', '=', 'cl.id_peg')->orOn('s.id_peg_rsm', '=', 'cl.id_peg')))
                    ->leftJoin('data_pegawai as dm',  'dm.rowid',  '=', 's.id_peg_dm')
                    ->leftJoin('data_pegawai as rsm', 'rsm.rowid', '=', 's.id_peg_rsm')
                    ->whereRaw('? BETWEEN s.periode_awal AND s.periode_akhir', [$monthyear])
                    ->whereIn('s.id_peg_mm', $idPegList)->whereNull('cl.approval')
                    ->whereBetween('cl.periode', [$startDate, $endDate])
                    ->where(fn($q) => $q->whereColumn('cl.id_peg', 's.id_peg_rsm')
                        ->orWhere(fn($q2) => $q2->whereColumn('cl.id_peg', 's.id_peg_mr')
                            ->whereIn('dm.status', ['Vacant', 'Dummy'])->whereIn('rsm.status', ['Vacant', 'Dummy']))
                        ->orWhere(fn($q2) => $q2->whereColumn('cl.id_peg', 's.id_peg_dm')->whereIn('rsm.status', ['Vacant', 'Dummy'])))
                    // ->count('cl.id'); 
                    ->distinct()->count('cl.id');
            }
        }
        ...
        if (str_contains($jabatan, 'ACT. DM') || str_contains($jabatan, 'DM')) {
            $planCount = DB::table('call_plan_actual as cl')
                ->join('struktur as s', fn($j) => $j->on('s.id_peg_mr', '=', 'cl.id_peg')->orOn('s.id_peg_dm', '=', 'cl.id_peg'))
                ->leftJoin('data_pegawai as rsm', 'rsm.rowid', '=', 's.id_peg_rsm')
                ->leftJoin('data_pegawai as mm',  'mm.rowid',  '=', 's.id_peg_mm')
                ->whereRaw('? BETWEEN s.periode_awal AND s.periode_akhir', [$monthyear])
        ...
        $cekActual = DB::table('call_setting as a')
            ->join('data_pegawai as b', 'a.user', '=', 'b.id_user')
            ->select('b.rowid', 'a.jumlah')
            ->whereIn('b.rowid', $idPegList)
            ->where('a.input_set', 'Approval Actual')->first();
        ...
        if (str_contains($jabatan, 'ACT. DM') || str_contains($jabatan, 'DM')) {
            $actualCount = DB::table('call_plan_actual as cl')
                ->join('struktur as s', fn($j) => $j->on('s.id_peg_mr', '=', 'cl.id_peg')->orOn('s.id_peg_dm', '=', 'cl.id_peg'))
                ->leftJoin('data_pegawai as rsm', 'rsm.rowid', '=', 's.id_peg_rsm')
                ->leftJoin('data_pegawai as mm',  'mm.rowid',  '=', 's.id_peg_mm')
                ->whereRaw('? BETWEEN s.periode_awal AND s.periode_akhir', [$monthyear])
